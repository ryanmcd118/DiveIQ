...
async function backfillUserNames() {
  try {
    ...
    const allUsers = await prisma.$queryRaw<
      Array<{ id: string; name?: string | null; firstName?: string | null }>
    >`
    ...
    if (!allUsers) {
      console.log(
        "Could not check database schema. This might mean migrations haven't been applied yet."
      );
    ...
    if (!hasNameField) {
      console.log(
        "✓ 'name' field doesn't exist in database - migrations have been applied."
      );
    ...
    if (usersToBackfill.length === 0) {
      console.log(
        "✓ No users need backfilling. All users already have firstName or no name field."
      );
      await prisma.$disconnect();
      return;
    }
    ...
    for (const user of usersToBackfill) {
      try {
        ...
        console.log(
          `✓ Updated user ${user.id}: "${user.name}" → firstName: "${firstName}", lastName: "${lastName}"`
        );
        successCount++;
      } catch (error) {
